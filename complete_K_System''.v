...
Lemma not_in_Formula_subst : forall p t' x, ~ x ∈ (term_Ens t')
  -> ~ x ∈ (Formula_Ens (p {x;; t'})).
Proof. Print term_Ens.
  intros. induction p; simpl; intros; auto.
  - apply union_complement_equiv. pose proof H.
    apply (not_in_term_subst t t') in H.
    apply (not_in_term_subst t0 t') in H0. auto.
  - eapply not_in_vector_subst in H; eauto.
  - apply union_complement_equiv. auto.
  - destruct (eqbv x v) eqn:E; simpl. intro.
Admitted.
