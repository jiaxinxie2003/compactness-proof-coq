...
Proof.
  induction t. 
  - intros c t' Hnot. simpl. reflexivity.
  - intros c' t' Hnot. simpl. simpl in Hnot.
    destruct (eqbc_reflect c' c) as [Heq | Hneq].
    + subst. exfalso. apply Hnot. rewrite Single. reflexivity.
    + reflexivity.
  - intros c t' Hnot. simpl. f_equal. simpl in Hnot.
    revert Hnot.
    (* 这里需要针对 Vector 的引理 *)
    admit.
Admitted.
...
Proof.
  ...
  - simpl. f_equal.
    + apply subst_tc_not_in. 
      (* 证明 c 不在第一个项中 *)
      intro H. apply Hnot. admit. (* 集合并集性质 *)
    + apply IHv.
      (* 证明 c 不在剩余向量中 *)
      intro H. apply Hnot. admit.
Admitted.
...
Proof.
  ...
  - revert y Hy_p Hy_Γ.
    ...
    + (* 情况 3: MP 规则 (p 是 q -> p 和 q 的结果) *)
      (* 需要引理：常量替换对蕴含号的分配律 *)
      (* Search ((_ → _) {/ _ ;; _ }) *)
      (* rewrite subst_c_dist_imp in IHHder1. *)
      eapply MP.
      * apply IHHp1. 
        -- (* 这里需要证明 y 不在 (q -> p) 的变元集中 *) admit.
        -- assumption.
      * apply IHHp2. admit.

    (* 如果 deduce_K_ind 包含 Gen 分支，需要处理 ∀ 的替换分配律 *)
Admitted.
